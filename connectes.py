...
def print_components_sizes(distance, points):
    ...
    with Perf("Global", True):
        ...
        # Tests
        segments = { 'groups': [], 'cluster_to_merge': [] }
        ...
        print(f"\n  Fin de la boucle principale : {Perf.times['Loop'][0]:7.4f}s\n")
        ...
        # Tests
        for point_id, nears in cluster_to_merge.items():
            for near_id in nears:
                segments['cluster_to_merge'].append((points[point_id], points[near_id]))
        # -----
    ...
    for near_id, referent_id in referents.items():
        groups[referent_id].add(near_id)

    test_rapport(points, cuts, distance, groups, segments)
    print(sorted(counts, reverse=True))
